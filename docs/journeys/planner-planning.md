# Journey: planner Planning (Interview → Plan)
...
planner acts as a planner: it interviews for clarity, performs targeted recon, explores alternatives, and produces an executable work plan (typically persisted under `.orchestrator/` when planning-with-files is enabled).

## End-to-End Flow

```mermaid
flowchart TD
  U["User request"] --> R["Recon (navigator/librarian)"]
  R --> Q["Interview (one question at a time)"]
  Q --> OPT["navigator 2-3 approaches (when non-trivial)"]
  OPT --> CLR{"Clearance check\\n(requirements, scope, tests)"}
  CLR -->|Pass| PLAN["Write plan spec → .orchestrator/plans/<planId>/plan.md"]
  CLR -->|Fail| Q
  PLAN --> EXEC["Execution (/start-work binds work.yaml + TaskGraph)"]
```

planner is the strategic planning agent in OpenCode. Named after the Titan who brought fire (knowledge/foresight) to humanity, it brings structure and clarity to complex work through thoughtful consultation.

## Overview

planner is a **planner, not an implementer**. It:
...
```
User Request → planner Interview → Design Doc → Work Plan → workflow-automator Execution Mode
```

## Architecture

planner is assembled from modular sections:

```
src/agents/planner/
├── index.ts                 # Assembly + exports
├── intent-taxonomy.ts       # Shared intent taxonomy (planner owner, scope-analyst validator)
...
├── high-accuracy-mode.ts    # Phase 3: reviewer iterative review (optional)
├── plan-template.ts         # Work plan structure template
└── behavioral-summary.ts    # Summary and cleanup rules
```

Runtime loading is interview-first (`PLANNER_RUNTIME_PROMPT`) with explicit phase handoff rules.
Full prompt (`PLANNER_SYSTEM_PROMPT`) remains available for full-context planning surfaces and tests.
...
| RECON FIRST | navigator codebase/docs before first question |
...
4. Design Validation (200-300 word sections, confirm each)
   ↓
5. Write Design Doc → .orchestrator/designs/{topic-slug}.md
   ↓
...
4. Parallel Probes, Synthesize Findings
   ↓
5. Write Findings → .orchestrator/drafts/{topic}-research.md
   ↓
6. Continue to Interview → Plan Generation
```
...
| Architecture | System design | Strategic, advisor consultation |
| Research | Investigation needed | Exit criteria, parallel probes |

The same taxonomy table is shared by scope-analyst so intent validation is aligned across the planning pipeline.
...
**Output:** `.orchestrator/plans/{planId}/plan.md` (execution-facing plan spec used directly by `/start-work`)

### Phase 3: High Accuracy Mode (Optional)

reviewer iterative review for stricter executability checks.
...
|----------|------|---------|
| Draft (active) | `.orchestrator/drafts/{topic}.md` | Working memory during interview |
| Draft (archived) | `.orchestrator/drafts/_archive/{planId}-{topic}.md` | Traceability after plan handoff |
| Design Doc | `.orchestrator/designs/{topic-slug}.md` | WHY/HOW decisions |
| Research Findings | `.orchestrator/drafts/{topic}-research.md` | Investigation results |
| Work Plan (spec) | `.orchestrator/plans/{planId}/plan.md` | Planner output and execution-facing plan document |
| TaskGraph (task SSOT) | `.orchestrator/tasks/plan/{planId}/task_*.json` | Task state, dependencies, revision (CAS) |
| Context Manifest | `.orchestrator/context-manifests/{planId}.md` | Deterministic context packs for delegation |
...
planner must end every turn with a valid action:
...
**planner CAN ONLY write:**
- `.orchestrator/drafts/*.md`
- `.orchestrator/designs/*.md`
- `.orchestrator/plans/*/plan.md`
- `.orchestrator/context-manifests/*.md`
...
OpenCode's planner is inspired by [Superpowers brainstorming](https://github.com/obra/superpowers) but adapted for the OpenCode ecosystem:

| Feature | Superpowers | planner |
|---------|-------------|------------|
...
| Recon first | No | Yes (navigator/librarian) |
| Research intent handling | No | Yes (modified flow) |
| High-accuracy review loop | No | Yes (reviewer iterative review) |
| Design/Plan separation | design.md → plan.md | .orchestrator/designs/ → .orchestrator/plans/ |
