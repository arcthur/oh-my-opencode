...
describe("findCaseInsensitive", () => {
  ...
  test("finds exact match first", () => {
    // given - object with exact and case-insensitive keys
    const obj = { advisor: "value1", Advisor: "value2" }

    // when - lookup with exact case
    const result = findCaseInsensitive(obj, "advisor")
...
describe("includesCaseInsensitive", () => {
  test("returns true for exact match", () => {
    ...
    const arr = ["navigator", "librarian"]

    // when - check exact match
    const result = includesCaseInsensitive(arr, "navigator")
  ...
  test("returns true for mixed case match", () => {
    ...
    const arr = ["advisor", "orchestrator"]

    // when - check different case
    const result = includesCaseInsensitive(arr, "advisor")
...
describe("findByNameCaseInsensitive", () => {
  test("finds element by exact name", () => {
    ...
    const arr = [{ name: "advisor", value: 1 }, { name: "navigator", value: 2 }]

    // when - find by exact name
    const result = findByNameCaseInsensitive(arr, "advisor")

    // then - returns matching element
    expect(result).toEqual({ name: "advisor", value: 1 })
  })
  ...
  test("finds element by case-insensitive name", () => {
    ...
    const result = findByNameCaseInsensitive(arr, "NAVIGATOR")

    // then - returns matching element
    expect(result).toEqual({ name: "navigator", value: 2 })
  })

  test("returns undefined when name not found", () => {
    // given - array without target name
    const arr = [{ name: "advisor", value: 1 }]
...
describe("equalsIgnoreCase", () => {
  test("returns true for same case", () => {
    ...
    // when - compare
    // then - returns true
    expect(equalsIgnoreCase("advisor", "advisor")).toBe(true)
  })
  ...
  test("returns true for different case", () => {
    ...
    // then - returns true
    expect(equalsIgnoreCase("advisor", "ADVISOR")).toBe(true)
    expect(equalsIgnoreCase("specialist", "specialist")).toBe(true)
  })
  ...
  test("returns false for different strings", () => {
    ...
    // when - compare
    // then - returns false
    expect(equalsIgnoreCase("advisor", "navigator")).toBe(false)
  })
})
