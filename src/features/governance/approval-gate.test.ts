...
import { createApprovalGateHook, type ApprovalGateConfig } from "./approval-gate"
import { ToolCriticalityRegistry, resetToolCriticalityRegistry } from "./tool-criticality"
import { InMemoryResumeTokenStore } from "./suspend-exception"
import { resetLedgerManager } from "./ledger"

const TEST_LEDGER_DIR = "/tmp/approval-gate-test-ledger"
...
describe("ApprovalGateHook", () => {
  ...
  beforeEach(() => {
    ...
    if (existsSync(TEST_LEDGER_DIR)) {
      rmSync(TEST_LEDGER_DIR, { recursive: true })
    }
  ...
  describe("ledger integration", () => {
    it("logs approval events to ledger", async () => {
      ...
      const ledgerPath = `${TEST_LEDGER_DIR}/session-1.jsonl`
