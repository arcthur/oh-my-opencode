...
describe("START_WORK_TEMPLATE", () => {
  test("documents work-state schema v4 fields and canonical plan paths", () => {
    // #given the builtin start-work template text
    const template = START_WORK_TEMPLATE

    // #then it should reflect work.yaml v4 invariants (no absolute plan paths)
    expect(template).toContain("schema_version: 4")
