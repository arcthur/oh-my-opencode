...
describe("mailbox/reader", () => {
  ...
  test("readUnreadByType combines filters", () => {
    createTestInbox([
      {
        id: "msg_001",
        from: "agent_other",
    ...
    const messages = readUnreadByType(teamName, agentId, "idle_notification", config)
    expect(messages.length).toBe(1)
    expect(messages[0].id).toBe("msg_002")
  })
  ...
  test("readInbox handles invalid JSON gracefully", () => {
    ...
    writeFileSync(join(inboxDir, "msg_invalid.json"), "invalid json content")
    ...
    writeFileSync(join(inboxDir, "msg_valid.json"), JSON.stringify(validMsg))
