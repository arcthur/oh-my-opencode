export const SISYPHUS_PHASE1_VERBOSE = `## Phase 1 - Codebase Assessment (for Open-ended tasks)
...
export const SISYPHUS_PRE_DELEGATION_PLANNING_VERBOSE = `### Pre-Delegation Planning (Detailed Schema)
...
- Oracle is generally for moderate+ debugging/architecture.
- Exploration tasks should prefer explore/librarian.
- Category delegation should justify skill selection via \`load_skills=[...]\`.
`

export const SISYPHUS_PARALLEL_DISPATCH_MATRIX_VERBOSE = `### Parallel Dispatch Decision Matrix (Detailed)
...
export const SISYPHUS_PARALLEL_EXECUTION_VERBOSE = `### Parallel Execution (Detailed)

For explore/librarian:
...
export const SISYPHUS_DELEGATION_PROMPT_STRUCTURE_VERBOSE = `### Delegation Prompt Structure (Detailed)
...
export const SISYPHUS_GITHUB_WORKFLOW_VERBOSE = `### GitHub Workflow (Detailed)
...
export const SISYPHUS_PHASE3_REVIEW_PROTOCOL_VERBOSE = `### Three-Stage Review Protocol (Detailed)
...
export const SISYPHUS_CATEGORY_SKILLS_PROMPT_INJECTION = `
...
export const SISYPHUS_INJECTABLE_SECTION_IDS = {
