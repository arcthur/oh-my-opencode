...
import { createSpecialistAgentWithOverrides, SPECIALIST_DEFAULTS } from "./specialist"

const TEST_SYSTEM_DEFAULT_MODEL = "google/gemini-3-pro"

describe("createSpecialistAgentWithOverrides", () => {
  describe("honored fields", () => {
    test("applies model override", () => {
      ...
      // when
      const result = createSpecialistAgentWithOverrides(override, TEST_SYSTEM_DEFAULT_MODEL)
  ...
  describe("defaults", () => {
    ...
    test("falls back to built-in default model when systemDefaultModel is missing", () => {
      ...
      // when
      const result = createSpecialistAgentWithOverrides(override, undefined)

      // then
      expect(result.model).toBe(SPECIALIST_DEFAULTS.model)
    })
    ...
    test("uses default temperature when no override", () => {
      ...
      // then
      expect(result.temperature).toBe(SPECIALIST_DEFAULTS.temperature)
    })
  })
  ...
  describe("tool safety (task blocked, delegate_task research-scoped)", () => {
    ...
    test("task remains blocked when using permission format override", () => {
      ...
      // when
      const result = createSpecialistAgentWithOverrides(override as Parameters<typeof createSpecialistAgentWithOverrides>[0], TEST_SYSTEM_DEFAULT_MODEL)
  ...
  describe("prompt composition", () => {
    test("base prompt contains discipline constraints", () => {
      ...
      // then
      expect(result.prompt).toContain("specialist")
      expect(result.prompt).toContain("You work ALONE")
