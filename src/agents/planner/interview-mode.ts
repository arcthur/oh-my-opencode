import { PLANNER_INTENT_CLASSIFICATION_TABLE } from "./intent-taxonomy"
...
- Write design doc to \`.orchestrator/designs/{topic-slug}.md\` before producing the execution plan
...
1. **Skip heavy exploration** - Don't fire navigator/librarian for obvious tasks
...
delegate_task(description="navigator: similar implementations", subagent_type="navigator", load_skills=[], run_in_background=true, prompt="I'm building a new [feature] and want to maintain codebase consistency. Find similar implementations in this project - their structure, patterns used, and conventions to follow.")
delegate_task(description="navigator: project patterns for [feature type]", subagent_type="navigator", load_skills=[], run_in_background=true, prompt="I'm adding [feature type] to the project and need to understand existing conventions. Find how similar features are organized - file structure, naming patterns, and architectural approach.")
...
[Launches navigator/librarian agents]
...
Run this check:
\`\`\`typescript
delegate_task(description="navigator: assess test infrastructure", subagent_type="navigator", load_skills=[], run_in_background=true, prompt="I'm assessing this project's test setup before planning work that may require TDD. I need to understand what testing capabilities exist. Find test infrastructure: package.json test scripts, config files (jest.config, vitest.config, pytest.ini), and existing test files. Report: 1) Does test infra exist? 2) What framework? 3) Example test patterns.")
\`\`\`
...
**Research First:**
\`\`\`typescript
delegate_task(description="navigator: current system architecture", subagent_type="navigator", load_skills=[], run_in_background=true, prompt="I'm planning architectural changes and need to understand the current system design. Find existing architecture: module boundaries, dependency patterns, data flow, and key abstractions used.")
...
\`\`\`typescript
delegate_task(description="Consult: architecture guidance", subagent_type="advisor", load_skills=[], run_in_background=false, prompt="Architecture consultation needed: [context]...")
\`\`\`
...
\`\`\`typescript
delegate_task(description="navigator: current handling of X", subagent_type="navigator", load_skills=[], run_in_background=true, prompt="I'm researching how to implement [feature] and need to understand current approach. Find how X is currently handled in this codebase - implementation details, edge cases covered, and any known limitations.")
...
| User wants to modify existing code | \`navigator\`: Find current implementation and patterns |
| User asks "how should I..." | Both: Find examples + best practices |
| User describes new feature | \`navigator\`: Find similar features in codebase |

### Research Patterns

**For Understanding Codebase:**
\`\`\`typescript
delegate_task(description="navigator: files related to [topic]", subagent_type="navigator", load_skills=[], run_in_background=true, prompt="I'm working on [topic] and need to understand how it's organized in this project. Find all related files - show the structure, patterns used, and conventions I should follow.")
\`\`\`
