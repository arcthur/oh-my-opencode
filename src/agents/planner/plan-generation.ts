 ...
 *
 * Phase 2: scope-analyst preflight, plan generation, reviewer review routing,
...
- Design doc written to \`.orchestrator/designs/{topic-slug}.md\`
- scope-analyst preflight completed for this planning cycle
...
task_create({ title: "plan-1 Consult scope-analyst for gap analysis (hard preflight)", scope: "session" })
task_create({ title: "plan-2 Choose plan name + assemble full planning context", scope: "session" })
task_create({ title: "plan-3 Generate work plan to .orchestrator/plans/{plan-id}/plan.md", scope: "session" })
task_create({ title: "plan-4 Generate context manifest to .orchestrator/context-manifests/{plan-id}.md", scope: "session" })
task_create({ title: "plan-5 Run reviewer plan review and apply blocking fixes", scope: "session" })
...
- Prevents skipping crucial steps like scope-analyst preflight and review
- Creates accountability for each phase
- Enables recovery if session is interrupted

**WORKFLOW:**
1. Phase 2 entered → **IMMEDIATELY** register tasks (plan-1 through plan-9)
2. Mark plan-1 as \`in_progress\` → Run scope-analyst hard preflight and incorporate directives
3. Mark plan-2 as \`in_progress\` → Pick plan name + assemble full context
4. Mark plan-3 as \`in_progress\` → Generate plan and write to \`.orchestrator/plans/{plan-id}/plan.md\`
5. Mark plan-4 as \`in_progress\` → Generate context manifest and write to \`.orchestrator/context-manifests/{plan-id}.md\`
6. Mark plan-5 as \`in_progress\` → Run reviewer review and apply blocking fixes
...
## Pre-Generation: scope-analyst Consultation (MANDATORY)

**BEFORE generating the plan**, you MUST consult scope-analyst for hard preflight.

\`\`\`typescript
delegate_task(
  description="Consult scope-analyst for gap analysis",
  subagent_type="scope-analyst",
  load_skills=[],
  run_in_background=false,
  prompt="Review this planning context before plan generation. Claimed intent from planner interview: [Trivial/Simple | Refactoring | Build from Scratch | Mid-sized Task | Collaborative | Architecture | Research]. Validate claimed intent first (MATCH or OVERRIDE with evidence), then identify missing constraints, ambiguous assumptions, and required guardrails."
)
\`\`\`

**Hard rule:**
- Do not generate \`.orchestrator/plans/{plan-id}/plan.md\` until scope-analyst preflight is complete.
- Incorporate scope-analyst directives before writing the plan.

## Plan Generation Path (Single Source of Truth)

After scope-analyst preflight:
1. Use the plan template and full interview context
2. Write to \`.orchestrator/plans/{plan-id}/plan.md\`
3. Generate context manifest at \`.orchestrator/context-manifests/{plan-id}.md\`
4. Continue to reviewer review and self-review

## Post-Generation: reviewer Review (Recommended)

After generating the plan:
1. Invoke reviewer to review the plan for blocking issues
\`\`\`typescript
delegate_task(description="Review plan for blocking issues", subagent_type="reviewer", load_skills=[], run_in_background=false, prompt="Review .orchestrator/plans/{plan-id}/plan.md for executability, missing dependencies, and unsafe assumptions")
\`\`\`
2. If reviewer returns [OKAY] → proceed to summary
3. If reviewer returns [REJECT] → address blocking issues, update plan, and re-run reviewer once

**Skip reviewer review** only for trivial/simple plans or if user explicitly requests skipping review.

## High Accuracy Review (Optional)

If user asks for high accuracy:
1. Run one additional reviewer review pass with stricter checklist:
   - missing edge cases
   - dependency ordering issues
   - unverifiable acceptance criteria
   - rollout/rollback blind spots
2. Apply fixes in \`.orchestrator/plans/{plan-id}/plan.md\`
3. Reconfirm with reviewer before handoff
...
**Generated Files:**
- Final Plan: \`.orchestrator/plans/{plan-id}/plan.md\`
  ...
  questions: [{
    ...
    options: [
      ...
      },
      {
        ...
        description: "Run one additional reviewer verification pass before handoff (slower, stricter)."
      }
    ]
  }]
})
...
- **High Accuracy Review** → Run stricter reviewer re-review, apply fixes, then guide to \`/start-work\`
