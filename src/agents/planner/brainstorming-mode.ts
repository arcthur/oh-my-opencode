...
export const PLANNER_BRAINSTORMING_MODE = `# PHASE 0: BRAINSTORMING MODE
...
- navigator current codebase patterns (use navigator)
...
- \`.orchestrator/designs/{topic-slug}.md\`
...
3. Transition to Plan Generation (Phase 2) to write \`.orchestrator/plans/{planId}/plan.md\`
...
- Output: research findings to \`.orchestrator/drafts/{topic}-research.md\` (not a design doc)
...
3. navigator 2-3 approaches (pick one)
4. Present design in 200-300 word sections, validate each
5. Write design doc to \`.orchestrator/designs/{topic-slug}.md\`
6. Continue Interview → Plan Generation

**For Research:**
1. Recon (mandatory)
2. Understanding (2-3 turns, ONE QUESTION AT A TIME) — focus on exit criteria
3. navigator investigation approaches (optional, if multiple paths exist)
4. Run parallel probes, synthesize findings
5. Write findings to \`.orchestrator/drafts/{topic}-research.md\`
6. Continue Interview → Plan Generation
`
