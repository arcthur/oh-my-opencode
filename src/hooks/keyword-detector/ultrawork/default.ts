...
\`\`\`
delegate_task(description="Plan tasks", subagent_type="plan", load_skills=[], run_in_background=false, prompt="<gathered context + user request>")
\`\`\`
...
| Scenario | Action |
|----------|--------|
| Plan agent asks clarifying questions | \`delegate_task(description="Continue planning", session_id="{returned_session_id}", load_skills=[], run_in_background=false, prompt="<your answer>")\` |
| Need to refine the plan | \`delegate_task(description="Refine plan", session_id="{returned_session_id}", load_skills=[], run_in_background=false, prompt="Please adjust: <feedback>")\` |
| Plan needs more detail | \`delegate_task(description="Expand plan", session_id="{returned_session_id}", load_skills=[], run_in_background=false, prompt="Add more detail to Task N")\` |
...
delegate_task(description="Plan tasks", subagent_type="plan", load_skills=[], run_in_background=false, prompt="Here's more info...")

// CORRECT: Resume preserves everything
delegate_task(description="Continue planning", session_id="ses_abc123", load_skills=[], run_in_background=false, prompt="Here's my answer to your question: ...")
\`\`\`
...
\`\`\`
// Frontend work
delegate_task(description="Implement UI", category="visual-engineering", load_skills=["frontend-ui-ux"], run_in_background=false, prompt="...")

// Complex logic
delegate_task(description="Solve hard problem", category="ultrabrain", load_skills=["typescript-programmer"], run_in_background=false, prompt="...")

// Quick fixes
delegate_task(description="Quick fix", category="quick", load_skills=["git-master"], run_in_background=false, prompt="...")
\`\`\`
...
3. **INVOKE PLAN AGENT** (input: gathered_context from step 2):
   \`\`\`
   result = delegate_task(description="Plan tasks", subagent_type="plan", load_skills=[], run_in_background=false, prompt="<gathered_context from step 2> + <user request>")
...
4. **ITERATE WITH PLAN AGENT** (if clarification needed):
   ...
   delegate_task(description="Continue planning", session_id=plan_session_id, load_skills=[], run_in_background=false, prompt="<answer to plan agent's question>")
   \`\`\`

5. **EXECUTE VIA DELEGATION** (category + skills from plan agent's output):
   \`\`\`
   delegate_task(description="Execute task", category="...", load_skills=[...], run_in_background=false, prompt="<task from plan>")
   \`\`\`
