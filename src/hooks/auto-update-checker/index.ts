...
export function createAutoUpdateCheckerHook(ctx: PluginInput, options: AutoUpdateCheckerOptions = {}) {
  const { showStartupToast = true, isOrchestratorEnabled = false, autoUpdate = true } = options

  const getToastMessage = (isUpdate: boolean, latestVersion?: string): string => {
    if (isOrchestratorEnabled) {
      return isUpdate
        ? `orchestrator on steroids is steering OpenCode.\nv${latestVersion} available. Restart to apply.`
        : `orchestrator on steroids is steering OpenCode.`
    }
    return isUpdate
  ...
  return {
    event: ({ event }: { event: { type: string; properties?: unknown } }) => {
      ...
      setTimeout(async () => {
        ...
        if (localDevVersion) {
          if (showStartupToast) {
            showLocalDevToast(ctx, displayVersion, isOrchestratorEnabled).catch(() => {})
          }
...
async function showSpinnerToast(ctx: PluginInput, version: string, message: string): Promise<void> {
  ...
  for (let i = 0; i < totalFrames; i++) {
    const spinner = ORCHESTRATOR_SPINNER[i % ORCHESTRATOR_SPINNER.length]
    await ctx.client.tui
      .showToast({
        body: {
...
async function showLocalDevToast(ctx: PluginInput, version: string | null, isOrchestratorEnabled: boolean): Promise<void> {
  const displayVersion = version ?? "dev"
  const message = isOrchestratorEnabled
    ? "orchestrator running in local development mode."
