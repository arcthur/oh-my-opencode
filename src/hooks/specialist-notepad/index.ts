...
export function createSpecialistNotepadHook(_ctx: PluginInput) {
  return {
    "tool.execute.before": async (
      ...
      // 3. Only inject for specialist executions:
      //    - explicit subagent_type="specialist"
      //    - category-based delegation (spawns specialist)
      const subagentType = output.args.subagent_type
      const hasCategory = typeof output.args.category === "string"
      const isSpecialist =
        typeof subagentType === "string" && subagentType.toLowerCase() === "specialist"

      if (!isSpecialist && !hasCategory) {
        return
      }
