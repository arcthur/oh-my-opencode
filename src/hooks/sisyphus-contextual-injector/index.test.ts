...
import { createSisyphusContextualInjectorHook } from "./index"
...
describe("sisyphus-contextual-injector hook", () => {
  beforeEach(() => {
    _resetForTesting()
  })
  ...
  test("injects github workflow context on matching chat.message", async () => {
    ...
    const hook = createSisyphusContextualInjectorHook(createMockCtx())
    const output = {
      message: {},
  ...
  test("does not inject for non-sisyphus sessions", async () => {
    // #given
    const sessionID = "session-non-sisyphus"
