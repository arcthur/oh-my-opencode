...
describe("createQuestionLabelTruncatorHook", () => {
  ...
  describe("tool.execute.before", () => {
    it("truncates labels exceeding 30 characters with ellipsis", async () => {
      ...
      // #then
      const truncatedLabel = (output.args as never as { questions: { options: { label: string }[] }[] }).questions[0].options[0].label;
    ...
    it("preserves labels within 30 characters", async () => {
      ...
      // #then
      const resultLabel = (output.args as never as { questions: { options: { label: string }[] }[] }).questions[0].options[0].label;
    ...
    it("handles multiple questions with multiple options", async () => {
      ...
      // #then
      const q1opts = (output.args as never as { questions: { options: { label: string }[] }[] }).questions[0].options;
      const q2opts = (output.args as never as { questions: { options: { label: string }[] }[] }).questions[1].options;
