...
import { createTeam, createAgentIdentity } from "../../../src/features/orchestrator-swarm/team/manifest"
...
import { signControlMessage } from "../../../src/features/orchestrator-swarm/security/control-message-signature"
import {
  createFsMailboxTransport,
  type MailboxTransport,
} from "../../../src/features/orchestrator-swarm/mailbox"
...
describe("agent/worker plan approval + mode control", () => {
  ...
  test("plan approval timeout resumes to WORKING (does not orphan task)", async () => {
    ...
    await waitFor(() => {
      expect(w.isWorking()).toBe(true)
    }, { timeoutMs: 3_000, intervalMs: 25, description: "worker enters working state" })
  ...
  test("mode acceptEdits causes requestPermission(Edit) to auto-approve without coordinator message", async () => {
    ...
    }, { timeoutMs: 3_000, intervalMs: 25, description: "worker switches to acceptEdits mode" })
  ...
  test("mode bypassPermissions causes requestPermission(Bash) to auto-approve without coordinator message", async () => {
    ...
    await waitFor(() => {
      expect(w.getCurrentMode()).toBe("bypassPermissions")
    }, { timeoutMs: 3_000, intervalMs: 25, description: "worker switches to bypassPermissions mode" })
  ...
  test("plan approval approval marks task as approved for plan-mode gate", async () => {
    ...
    await waitFor(() => {
      expect(w.getCurrentMode()).toBe("plan")
    }, { timeoutMs: 3_000, intervalMs: 25, description: "worker switches to plan mode" })
    ...
    await waitFor(() => {
      expect(readByType(teamName, coordinator.id, "plan_approval_request", config).length).toBeGreaterThan(0)
    }, { timeoutMs: 2_000, intervalMs: 25, description: "coordinator receives plan approval request" })
  ...
  test("ignores mode_set_request from non-coordinator sender", async () => {
    ...
    }, { timeoutMs: 1_500, intervalMs: 25, description: "worker rejects spoofed mode_set_request" })
  ...
  test("records late permission_response after timeout", async () => {
    ...
    await waitFor(() => {
      expect(readByType(teamName, coordinator.id, "permission_request", config).length).toBeGreaterThan(0)
    }, { timeoutMs: 2_000, intervalMs: 25, description: "coordinator receives permission request" })
    ...
    await waitFor(() => {
      ...
      expect(readdirSync(lateDir).some((name) => name.endsWith(".json"))).toBe(true)
    }, { timeoutMs: 3_000, intervalMs: 25, description: "late response diagnostics file written" })
  ...
  test("rejects unsigned privileged message when enforce_signature is enabled", async () => {
    ...
    }, { timeoutMs: 1_500, intervalMs: 25, description: "signature validation rejects unsigned privileged message" })
  ...
  test("accepts signed privileged message when enforce_signature is enabled", async () => {
    ...
    await waitFor(() => {
      expect(w.getCurrentMode()).toBe("acceptEdits")
    }, { timeoutMs: 3_000, intervalMs: 25, description: "signed privileged message accepted" })
