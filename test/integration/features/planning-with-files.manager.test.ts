...
import {
  ...
  writeLedger,
  writePlan,
} from "../../../src/features/planning-with-files/manager"
import { createWorkStateManager } from "../../../src/features/work-state"
import { DEFAULT_PLANNING_CONFIG } from "../../../src/features/planning-with-files/types"
...
describe("planning-with-files manager", () => {
  ...
  test("detectActivePlan falls back to latest modified plan", async () => {
    ...
    await initializePlan(tmpDir, "plan-a", "A")
    await initializePlan(tmpDir, "plan-b", "B")
    const planAPath = getExecutionPlanPath(tmpDir, "plan-a")
    const planBPath = getExecutionPlanPath(tmpDir, "plan-b")
    const now = new Date()
    const newer = new Date(now.getTime() + 5_000)
    fs.utimesSync(planAPath, now, now)
    fs.utimesSync(planBPath, newer, newer)
